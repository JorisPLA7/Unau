...
def startGame():
    ...
    ##Titre
    header = Label(root, text="C'est au tour du joueur {}".format(a.active))
    ...
    for i in range(len( a.player[a.active].hand)):
        textVal = "| {} : ~ {} de {} ~ |".format(i, a.player[a.active].hand[i].val, a.player[a.active].hand[i].typ)
        hand = Button(root, text=textVal)
        hand.pack()

    

    '''
    titreangleinter = Label(subinteractif1, text="angle entre deux positions (degrés) :")
    titreangleinter.pack()  # on intègre le module déclaré à sa fenêtre (pack(sans paramètre) donc simplement à la suite du reste)

    saisieangleinter = Spinbox(subinteractif1, from_=0, to=50)
    ...
    checkbutton = Button(root, text="Verifier", )
